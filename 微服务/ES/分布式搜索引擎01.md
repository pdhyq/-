...
回忆：如果对某个字段进行了模糊匹配，那么不管这个字段有没有建立索引，都是逐行扫描
...
安装es和kibana时，之所以要创建一个网络，是因为docker容器之间是隔离的，所以要创建一个网络，[使用Docker实现容器之间的互通_docker两个容器互通-CSDN博客](https://blog.csdn.net/m0_67094505/article/details/128294865)
...
注意，es中是没有数组类型的，但是允许每个类型有多个值，所以最终的类型就是float
...
这里在加上这两个注解之后，每当运行一个测试方法都会先运行BeforeEach的方法，最后运行AfterEach的方法
