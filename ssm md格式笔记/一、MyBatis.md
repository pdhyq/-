一个映射文件对应是查一张表，所以如果要查不同的单独的两张表，就用两个映射文件
...
原因是这里的引入映射文件方式，正确方式是`<package name="mapper"/>`或者是`<mapper resource="mapper/UserMapper.xml"/>`
...
>     -->
>     <!--引入properties文件，此后就可以用${key}的方式访问value-->
...
>             属性：
>                 type：设置需要设置别名的类型
>                 alias：设置某个类型的别名，若不设置该属性，那么该类型拥有默认的别名，就是类名，且不区分大小写
>         -->
...
>                  POOLED：表示使用数据库连接池缓存数据库连接（比如durid）
...
> 可以通过@Param注解标识mapper接口中的方法参数，此时，会将这些参数放在map集合中，并以两种方式储存
>
...
1. PageHelper代表的是开启分页的功能
2. PageInfo获取分页的相关数据，其中的泛型填的是当前查询的数据要转换的实体类类型
3. 