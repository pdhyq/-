...
人话：通过bean的类型，bean所继承的类型，bean所实现的接口都可以用来获取bean

#### ⑥疑问解答

上述所说的结论的时机是在获取bean的时候，那么在配置bean的时候，就是在xml中的bean标签中的class可以用接口吗？

答案是不能，因为最终获取bean的方式就是调用这里设置的类的构造函数，一个接口都不能实例化没有构造函数怎么可以，所以这里边写的只能是具体的类型，包括抽象类也不行。
...
比如上述情况，如果按照常规的方法，最后一个参数是24，他既可以赋值给age，又可以赋值给score，默认是赋值给了score。若想赋值给age，就要用到name属性，用来指定给哪个属性赋值

```xml
    <bean id="studentTwo" class="pojo.Student">
        <constructor-arg value="2"/>
        <constructor-arg value="甘雨"/>
...
第二种方法：配置一个集合类型的bean，需要使用util的约束

```xml
    <bean id="clazzTwo" class="pojo.Clazz">
        <property name="cid" value="1"/>
        <property name="cname" value="36班"/>
        <!--
        <property name="students">
            <list>
                <ref bean="studentOne"></ref>
...
解释：首先是为什么要在前边加个util，util是一个约束。因为要区分不同的list标签，毕竟其他的约束里边也有这list标签。第二个问题class属性为什么不写，因为这是集合类型，目的是为了往里边添加数据，换句话说你已经知道了这是集合类型，而上面的bean标签是给类型里边的属性赋值，而不是添加值，所以要用其他的标签，且不用说明类型，因为既然你用了这个标签，就已经知道你要操作的对象是集合类型，要干的事是往里添加数据
...
简单来说，就是将FactoryBean的实现类配置为bean时，会将当前类中的getObject（）所返回的对象交给IOC容器管理
...
**注解的作用是它标记的类在IOC容器中自动配置了当前类型的bean，而bean的类型不能是接口，只能是类，所以注解不能添加在接口上，只能添加在实现类上**
...
```xml
<context:component-scan base-package="com.atguigu">
    <!-- context:exclude-filter标签：指定排除规则，排除扫描 -->
    <!--
        type：设置排除或包含的依据
...
解释：非核心业务代码在目标对象中叫做横切关注点，将其抽取出来放到切面类（下边有定义）中进行封装，此时的横切关注点在切面中表示为一个通知方法。横切关注点是相对于目标对象来说的，通知方法是相对于切面类说的

- 前置通知：在被代理的目标方法**前**执行
- 返回通知：在被代理的目标方法**成功结束**后执行（**寿终正寝**）
- 异常通知：在被代理的目标方法**异常结束**后执行（**死于非命**）
- 后置通知：在被代理的目标方法**最终结束**后执行（**盖棺定论**）
- 环绕通知：使用try...catch...finally结构围绕**整个**被代理的目标方法，包括上面四种通知对应的所有位置<img src="img\21.png" alt="21" style="zoom:50%;" />

#### ③切面

封装通知方法的类。<img src="img\22.png" alt="22" style="zoom:50%;" />
...
使用AOP之后，IOC容器无法管理目标对象，只能创建代理对象来间接管理目标对象
...
public class LoggerAspect {
    @Pointcut("execution(* annotation.Calculatorlmpl.*(..))")
    public void pointcut(){}

    //@Before("execution(public int annotation.Calculatorlmpl.add(int,int))")
    //@Before("execution(* annotation.Calculatorlmpl.*(..))")
    @Before("pointcut()")
    public void beforeAdviceMethod(JoinPoint joinPoint){
        System.out.println("前置通知");
        Signature signature = joinPoint.getSignature();
        System.out.println("参数名："+signature.getName()+"，参数为："+ Arrays.toString(joinPoint.getArgs()));
    }
    @After("pointcut()")
    ...
    就可以将通知方法的某个参数指定为接受目标对象的方法的返回值参数
    */
...
bean标签中可以不写id，如果可以直接通过类名获取，p107 11:00
...
```java
//指定当前测试类在Spring的测试环境中运行，此时就可以通过注入的方式直接获取IOC容器中的bean
...
```java
    ...
    public void testInsert(){
        ...
        jdbcTemplate.update(sql,null,"Apple","123","它","kuke@apple.com");
    }
```
...
```java
    ...
    public void testSelect(){
        ...
        User query = jdbcTemplate.queryForObject(sql, new BeanPropertyRowMapper<>(User.class), 8);
