...
MVC的工作流程： 用户通过视图层发送请求到服务器，在服务器中请求被Controller接收，Controlle调用相应的Model层处理请求，处理完毕将结果返回到Controller，Controller再根据请求处理的结果找到相应的View视图，渲染数据后最终响应给浏览器
...
？：表示任意的单个字符，不包括？和/，因为在一条url路径中，？后边代表的是携带的参数，(www.baidu.com?username=root)

*：表示任意的0个或多个字符，不包括？和/，/表示路径分隔符
...
```java
    @RequestMapping("/")
    public String portal(@RequestParam(value = "username",required = false) String username,
                         String password,
...
注意因为默认是请求参数和控制器方法的参数建立映射，所以如果控制器方法参数要和请求头信息和cookie数据建立映射时，一定要用注解。
...
首先要明确一点，在你获取请求参数的时候，DispatcherServlet已经在内部自动获取了，请求的参数都来自于springmvc框架自动获取的这个request。所以在控制器方法里边改变编码是没有用的，因为早就获取完了。不懂就返回来看看网课，逻辑有点绕
...
> 注：
>
> SpringMVC中处理编码的过滤器一定要配置到其他过滤器之前，否则无效，原因就是设置编码之前，一定不能获取请求参数，如果获取过了，那么就不会有效果了
...
**转发视图用的少，因为转发的视图不能进行thymeleaf的渲染，如果转发的就是个thymeleaf视图，那还不如直接访问所以没有必要**
...
![image-20231014230126010](img/image-20231014230126010.png)

list->JSON数组

![image-20231014230202164](img/image-20231014230202164.png)
...
> 补充知识：如果RequestMapping注解所对应的控制器方法，返回值是void，那就会把RequestMapping中的请求地址作为逻辑视图返回
...
<mvc:interceptor>
    <mvc:mapping path="/**"/>   /*表示只拦截上下文路径下第一层路径，ant风格
...
> 补充知识：SpringMVC的IOC容器是子容器，Spring的IOC容器是父容器，子容器可以访问父容器的bean，父容器不能访问子容器的bean
