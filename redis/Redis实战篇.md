...
[BeanUtils中copyProperties的使用](https://blog.csdn.net/J_bean/article/details/78737492)
...
每个不同的浏览器发送的请求都有一个独立的session，tomcat的内部就有很多的session，所以不同用户用session存储数据时，都用”code“作为key，不会发生冲突。tomcat又怎么知道你的session在哪呢？	创建session时，就会自动形成sessionid，写到用户浏览器的cookie中，每次用户浏览器发请求就从cookie中取出sessionid就能找到对应的session了
...
* 先操作缓存还是先操作数据库？（先搞清楚目的，是为了让数据库和缓存的数据一致，不过中途有没有人脏读了，只要更新完数据之后，能保持一致性就好了）
  * 先删除缓存，再操作数据库
  * 先操作数据库，再删除缓存
...
如果你选择方案二，需要恰好缓存意外失效，然后去查询数据库，然后写入缓存，而就在你写入缓存的这一段时间，数据库恰好更新，然后再去删除缓存。但是数据库的更新是比写入缓存慢上很多的，所以这个操作发生的概率特别低
...
我们把过期时间设置在 redis的value中，注意：这个过期时间并不会直接作用于redis，而是我们后续通过逻辑去处理。假设线程1去查询缓存，然后从value中判断出来当前的数据已经过期了，此时线程1去获得互斥锁，那么其他线程会进行阻塞，**获得了锁的线程他会开启一个 线程去进行 以前的重构数据的逻辑，直到新开的线程完成这个逻辑后，才释放锁， 而线程1直接进行返回过期的数据**，假设现在线程3过来访问，由于线程线程2持有着锁，所以线程3无法获得锁，线程3也直接返回过期的数据，只有等到新开的线程2把重建数据构建完后，其他线程才能走返回正确的数据。（摆烂，佛系）
...
**互斥锁方案：**由于保证了互斥性，所以数据一致，且实现简单，因为仅仅只需要加一把锁而已，也没其他的事情需要操心，所以没有额外的内存消耗，缺点在于有锁就有死锁问题的发生，且只能串行执行性能肯定受到影响（当上了锁之后，其余的线程都要等待，直到锁释放）
...
思路分析：当用户开始查询redis时，判断是否命中，如果没有命中则直接返回空数据，不查询数据库，而一旦命中后，将value取出，判断value中的过期时间是否满足，如果没有过期，则直接返回redis中的数据，如果过期，则在开启独立线程后直接返回之前的数据，独立线程去重构数据，重构完成后释放互斥锁。**（这个过程中就看缓存有没有命中了，命中了就会返回不管过没过期，没命中就直接返回null）**
...
intern() 这个方法是从常量池中拿到数据，如果我们直接使用userId.toString() 他拿到的对象实际上是不同的对象，new出来的对象，地址就不一样，和对象本身的值没关系，我们使用锁必须保证锁必须是同一把，所以我们需要使用intern()方法。要区分开对象和值
...
但是以上做法依然有问题，因为你调用的方法，其实是this.的方式调用的，事务想要生效，还得利用代理来生效，所以这个地方，我们需要获得原始的事务对象， 来操作事务**（aop失效）**
...
最后面为什么是Boolean.TRUE.equals(success)，而不是直接返回，因为要装箱拆箱，如果Boolean是空指针的话，拆箱就会出错，这样返回如果是空就直接返回false。
...
这样会有问题，原有持锁线程其实是和新持锁线程产生竞态条件了，因此执行业务代码可能会产生异常
...
![image-20231120095102378](img/image-20231120095102378.png)
...
虽然不在内存中保存，但是当发送数据时，如果有消费者监听，会在消费者那里有一个缓存区域将消息缓存下来，如果消费者处理消息比较慢，然后有大量的消息需要被缓存，而消费者客户端的缓存是有上限的，一旦超出就会丢失。
...
@TableField(exist = *false*) 代表在实体类中，该注解标识的字段不是当前的实体类所映射的表
...
>
> ![image-20231127111116335](img/image-20231127111116335.png)
>
> 上面转成哈希是自己瞎写的。
